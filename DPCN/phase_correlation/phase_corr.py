...
# imshow(a.squeeze(0).float())
    ...
    G_a = torch.fft.fft2(a)
    G_b = torch.fft.fft2(b)    
    eps=1e-15

    real_a = torch.real(G_a)
    real_b = torch.real(G_b)
    imag_a = torch.imag(G_a)
    imag_b = torch.imag(G_b)

# compute a * b.conjugate; shape=[B,H,W,C]
    # R = torch.FloatTensor(max(G_a.shape[0],G_b.shape[0]), G_a.shape[1], G_a.shape[2], 2).to(device)
    # R[:, :, :, 0] = real_a * real_b + imag_a * imag_b
    # R[:, :, :, 1] = real_a * imag_b - real_b * imag_a
    R_c0 = real_a * real_b + imag_a * imag_b
    R_c1 = real_a * imag_b - real_b * imag_a
    R = torch.stack([R_c0, R_c1],dim=-1)

    r0 = torch.sqrt(real_a ** 2 + imag_a ** 2 + eps) * torch.sqrt(real_b ** 2 + imag_b ** 2 + eps)
    R[:, :, :, 0] = R[:, :, :, 0]/(r0 + eps)#.to(device)
    R[:, :, :, 1] = R[:, :, :, 1]/(r0 + eps)#.to(device)

    r = torch.fft.ifft2(torch.complex(R[:, :, :, 0],R[:, :, :, 1]))
    r_real = torch.real(r)
    r_imag = torch.imag(r)
...
# feed the result of phase correlation to the NET
    softargmax_input = modelc2s(r)
# suppress the output to angle and scale
    angle_resize_out_tensor = torch.sum(softargmax_input, 2, keepdim=False)
    scale_reszie_out_tensor = torch.sum(softargmax_input, 1, keepdim=False)
    # softargmax_input_np = cv2.applyColorMap((softargmax_input[0]*255).detach().cpu().numpy().astype(np.uint8), cv2.COLORMAP_JET)
    # cv2.imwrite('demo/softargmax_input.png',softargmax_input_np)

# get the argmax of the angle and the scale
    angle_out_tensor = torch.argmax(angle_resize_out_tensor[:,128:128+64], dim=-1) + 128
    # angle_out_tensor = torch.argmax(angle_resize_out_tensor, dim=-1)
    scale_out_tensor = torch.argmax(scale_reszie_out_tensor, dim=-1)

#calculate angle
    angle_ori = angle_out_tensor*180.00/r.shape[1]
    angle = angle_ori.clone()
    angle[angle_ori>=90] -= 90
    angle[angle_ori<90] += 90.00
...
# calculate scale
    ...
    sca_f = scale_out_tensor-r.shape[2] // 2
    scale = 1 / torch.pow(logbase, sca_f.float())#logbase ** sca_f

    return [angle, scale,r]
...
#     R_c0 = real_a * real_b + imag_a * imag_b
#     R_c1 = real_a * imag_b - real_b * imag_a
#     R = np.stack([R_c0, R_c1],axis=-1)

#     r0 = np.sqrt(real_a ** 2 + imag_a ** 2 + eps) * np.sqrt(real_b ** 2 + imag_b ** 2 + eps)
#     R[:, :, :, 0] = R[:, :, :, 0]/(r0 + eps)#.to(device)
#     R[:, :, :, 1] = R[:, :, :, 1]/(r0 + eps)#.to(device)

#     r = np.fft.ifft2(R[:, :, :, 0]+1j*R[:, :, :, 1])
#     r_real = np.real(r)
#     r_imag = np.imag(r)
#     r = np.sqrt(r_real ** 2 + r_imag ** 2 + eps)
#     r = np.fft.fftshift(r)
#     if trans:
#         r[:,0:60,:]=0.
#         r[:,G_a.shape[1]-60:G_a.shape[1], :] = 0.
...
#     # plt.show()
# # feed the result of phase correlation to the NET
#     softargmax_input = modelc2s(torch.tensor(r))
# # suppress the output to angle and scale
#     angle_resize_out_tensor = torch.sum(softargmax_input, 2, keepdim=False)
#     scale_reszie_out_tensor = torch.sum(softargmax_input, 1, keepdim=False)
#     # softargmax_input_np = cv2.applyColorMap((softargmax_input[0]*255).detach().cpu().numpy().astype(np.uint8), cv2.COLORMAP_JET)
