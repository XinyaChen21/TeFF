...
def detect_model(template_path, source_path, model_template, model_source, model_corr2softmax,\
    ...
    best_mse_loss = 10086
    for channal_idx in range(3):
        with torch.no_grad():

            iters = 0
            acc = 0.
            template_dino, _, _, _, _, _ = default_loader(template_path, 256,  mask=False,gray=False,channal_num=3)
            source_dino, _, _, _, _, _ = default_loader(source_path, 256, mask=True,gray=False,channal_num=3)
            template, _, _, _, _, _ = default_loader(template_path, 256,  mask=False,gray=False,channal_num=1,channal_idx=channal_idx)
            source, _, _, _, _, _ = default_loader(source_path, 256, mask=True,gray=False,channal_num=1,channal_idx=channal_idx)
            ...
            image_aligned = align_image(template[0,:,:], source_rotated[0,:,:])

            source_rotated_dino = kornia.warp_affine(source_dino[None].to(device), rot_mat, dsize=(h, w))
            ...
            mse_loss = ((source_rotated_dino[0]-template_dino.to(device))**2).mean().detach().cpu().numpy()
        best_mse_loss = min(best_mse_loss, mse_loss)
    mse_loss_orimg = ((source_dino-template_dino)**2).mean()

    return best_mse_loss, mse_loss_orimg
...
for idx in range(5):
    best_mse_loss=10086
    best_tempalate_idx = -1

    best_mse_loss_orimg=10086
    best_tempalate_idx_orimg = -1
    ...
    for i in range(72, 7*36):
        ...
        if not os.path.exists(template_path) or not os.path.exists(source_path):
            continue
        mse_loss, mse_loss_orimg=detect_model(template_path, source_path, model_template, model_source, model_corr2softmax, model_trans_template, model_trans_source, model_trans_corr2softmax, use_unet=False)
        if mse_loss<best_mse_loss:
            best_mse_loss=mse_loss
            best_tempalate_idx=i
        print(mse_loss)
        if mse_loss_orimg<best_mse_loss_orimg:
            best_mse_loss_orimg=mse_loss_orimg
            best_tempalate_idx_orimg=i

    print(best_tempalate_idx)
    print(best_mse_loss)
