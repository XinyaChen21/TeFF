...
def detect_model(template_path, source_path, model_template, model_source, model_corr2softmax,\
    ...
    with torch.no_grad():
        ...
        iters += 1    
        since = time.time()
        rotation_cal, scale_cal = detect_rot_scale(template.repeat(1,1,1,1), source.repeat(256,1,1,1),model_template, model_source, model_corr2softmax, device, use_unet=use_unet )
        # rotation_cal, scale_cal = detect_rot_scale(template.repeat(256,1,1,1), source,\
        ...
        # plot_and_save_result(template[0,:,:], source[0,:,:], source_rotated[0,:,:], image_aligned, save_path=os.path.join('demo', os.path.basename(source_path).replace('npy', 'png')))
    ...
    return mse_loss, mse_loss_orimg
