...
def detect_model(template_path, source_path, model_template, model_source, model_corr2softmax,\
    ...
    with torch.no_grad():
        ...
        iters = 0
        acc = 0.
        template_dino, _, _, _, _, _ = default_loader(template_path, 256,  mask=False,gray=False)
        source_dino, _, _, _, _, _ = default_loader(source_path, 256, mask=True,gray=False)
        template, _, _, _, _, _ = default_loader(template_path, 256,  mask=False)
        source, _, _, _, _, _ = default_loader(source_path, 256, mask=True)
...
for idx in range(5):
    ...
    for i in range(72, 7*36):
        ...
        template_path = os.path.join(f'/home/chenxinya/eg3d_pose_clean/training-runs/00435-comp-comp-gpu8-bs32-gm1-DC1-ali0.0-pc3-2D-bgTrue-sed10086-unfFalse-lvrFalse-vth0.0-gpcFalse-rdalFalse-itgrl0.0-itth0.0-flpTrue-lktfvFalse-dc2-avgFalse-dnTrue-sel-mulFalse-tem1.0t10.0-h1.0-DpFalse-dnspFalse-dtchFalse-var0.0-min0.01-tv0.0-probFalse/network-snapshot-001000_{i}.npy')
        source_path = os.path.join('/nas3/vilab/xychen/dino_feature/compcars_dinov1_stride4_pca16',str(idx).zfill(10)+'.npy')

        if not os.path.exists(template_path) or not os.path.exists(source_path):
            continue
        mse_loss=detect_model(template_path, source_path, model_template, model_source, model_corr2softmax, model_trans_template, model_trans_source, model_trans_corr2softmax, use_unet=False)
