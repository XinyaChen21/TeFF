...
def detect_rot_scale(template_rot, source_rot, model_template_rot, model_source_rot, model_corr2softmax_rot, device, use_unet=True):
    print("                             ")
    ...
    if use_unet:
        ...
        template_unet_rot = template_rot
        source_unet_rot = source_rot
    ...
    print(f'fft time is {time_elapsed}')

    # cv2.imwrite('demo/debug_source_fft.png',source_fft[0].detach().cpu().numpy())
    ...
    time_elapsed = time.time() - since
    print(f'filter time is {time_elapsed}')
    # cv2.imwrite('demo/debug_source_fft_logpolar_filter.png',source_fft[0].detach().cpu().numpy()*20)
    # template_fft = template_fft.squeeze(-1) 
    # source_fft = source_fft.squeeze(-1)
    ...
    time_elapsed = time.time() - since
    print(f'template logpolar time is {time_elapsed}')
    source_logpolar, logbase_rot = logpolar_layer(source_fft)
    time_elapsed = time.time() - since
    print(f'source logpolar time is {time_elapsed}')
    # cv2.imwrite('demo/debug_source_logpolar.png',source_logpolar[0].detach().cpu().numpy()*20)
    ...
    time_elapsed = time.time() - since
    print(f'pc time is {time_elapsed}')
...
def detect_translation(template_trans, source_trans, rotation, scale, model_template_trans, model_source_trans, model_corr2softmax_trans, device, use_unet=True ):
    print("                             ")
...
# for AGDatase
    ...
    # image = source_trans.detach().cpu().numpy()
    # image = cv2.warpAffine(image[0,0][...,None], rot_mat[0].detach().cpu().numpy(), (w, h), borderValue=(1))
    # source_trans = torch.tensor(image[None,None]).to(device)
    ...
    if use_unet:
        ...
        template_unet_trans = template_trans
        source_unet_trans = source_trans
