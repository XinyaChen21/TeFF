...
def warp_img1_to_img0(depth0, intrinsic, c2w0, w2c1, img1):
    ...
    # xy1 = torch.round(xy1).long()
    # xy1[:, :, 0] = torch.clip(xy1[:, :, 0], 0, W - 1)
    # xy1[:, :, 1] = torch.clip(xy1[:, :, 1], 0, H - 1)
    # xy1 = xy1.unsqueeze(1).repeat(1,3,1,1).reshape(N,3,H,W,2)

    # # create the target image
    # img1 = torch.zeros_like(img0)
    # rgb0 = img0#.reshape(N, 3, H * W)
    # print(img1.shape, rgb0.shape, xy1.shape)
    # img1[:, :, xy1[:, :, 1], xy1[:, :, 0]] = rgb0

    # # create the target mask
    # mask1 = torch.zeros_like(img0)[:, 0]
    # mask1[:, xy1[:, :, 1], xy1[:, :, 0]] = 1
