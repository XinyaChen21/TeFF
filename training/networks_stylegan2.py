...
@persistence.persistent_class
class MappingNetworkObj(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality, 0 = no latent.
        c_dim,                      # Conditioning label (C) dimensionality, 0 = no label.
        w_dim,                      # Intermediate latent (W) dimensionality.
...
class MappingNetworkPose(torch.nn.Module):
    ...
    def forward(self, z=None, c=None, truncation_psi=1, truncation_cutoff=None, update_emas=False, clamp_h=None, clamp_v=None, h_mean=math.pi/2, r_mean=2.7, fov_mean=20, lookat_h_mean=math.pi/2, lookat_v_mean=math.pi/2, lookat_radius_mean=0.3):
        # Embed, normalize, and concat inputs.
        x = None
        with torch.autograd.profiler.record_function('input'):
            if self.z_dim > 0:
...
class Generator(torch.nn.Module):
    def __init__(self,
        ...
        **synthesis_kwargs,         # Arguments for SynthesisNetwork.
        ...
        mapping_obj_kwargs = copy.deepcopy(mapping_kwargs)
        if gen_cond:
            self.mapping_obj = MappingNetwork(z_dim=z_dim, c_dim=c_dim, w_dim=w_dim, num_ws=self.num_ws, **mapping_obj_kwargs)
        else:
            self.mapping_obj = MappingNetworkObj(z_dim=z_dim, c_dim=c_dim, w_dim=w_dim, num_ws=self.num_ws, **mapping_obj_kwargs)

    def forward(self, z, c, truncation_psi=1, truncation_cutoff=None, update_emas=False, **synthesis_kwargs):
        ws = self.mapping_obj(z, c, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, update_emas=update_emas)
    ...
    def __init__(self,
        ...
        if self.dis_cam_weight > 0:
            if self.dis_linear_pose:
                self.cam_out = FullyConnectedLayer(in_channels * (resolution ** 2), dis_cam_dim, activation='linear')
            else:
                self.cam_out1 = FullyConnectedLayer(in_channels * (resolution ** 2), in_channels * (resolution ** 2)//2, activation='lrelu')
                self.cam_out2 = FullyConnectedLayer(in_channels * (resolution ** 2)//2, dis_cam_dim, activation='linear')
