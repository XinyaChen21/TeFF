...
class SingleDiscriminator(torch.nn.Module):
    ...
    def forward(self, img, c, update_emas=False, **block_kwargs):
        ...
        results = self.b4(x, img, cmap)
        return {'score': results['score'], 'cam': results['cam']}
...
@persistence.persistent_class
class SingleDiscriminator_1(torch.nn.Module):
    def __init__(self,
        c_dim,                          # Conditioning label (C) dimensionality.
        img_resolution,                 # Input resolution.
        img_channels,                   # Number of input color channels.
    ...
    def forward(self, img, c, update_emas=False, cam_pred=False, **block_kwargs):
        img = img['image']
        ...
        cmap = None
        if self.c_dim > 0 and (not cam_pred):
...
@persistence.persistent_class
class SingleDiscriminator_posecond(torch.nn.Module):
    def __init__(self,
        c_dim,                          # Conditioning label (C) dimensionality.
        img_resolution,                 # Input resolution.
        img_channels,                   # Number of input color channels.
...
class DualDiscriminator(torch.nn.Module):
    ...
    def forward(self, img, c, update_emas=False, cam_pred=False, **block_kwargs):
        ...
        # dino_raw = filtered_resizing(img['dino_raw'], size=img['image'].shape[-1], f=self.resample_filter)
        img = torch.cat([img['image'], image_raw], 1)

        # dino_raw_np = dino_raw.detach().cpu().numpy().transpose(0,2,3,1)[0]
        # image_raw_np = image_raw.detach().cpu().numpy().transpose(0,2,3,1)[0]
        # visual_img = np.hstack([dino_raw_np, image_raw_np])*127.5 + 127.5
...
class DinoDiscriminator(torch.nn.Module):
    ...
    def forward(self, img, c, update_emas=False, cam_pred=False, detach_img=True, **block_kwargs):
        if detach_img:
            img = torch.cat([img['image_raw'].detach(), img['dino_raw']], 1)
        else:
            img = torch.cat([img['image_raw'], img['dino_raw']], 1)
